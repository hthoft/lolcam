...
Config.instname = "Aalborg University"
Config.profilename = "eduroam"
Config.url = "http://www.wifi.aau.dk/"
Config.title = "eduroam CAT"
Config.server_match = "wifi.aau.dk"
...
Config.user_realm = "aau.dk"
Config.ssids = ['eduroam']
Config.del_ssids = ['AAU']
Config.servers = ['DNS:wifi.aau.dk']
...
"""Added by AAU"""
def aau_5Ghz_capable (interface):
    try:
        output = subprocess.check_output(["iwlist", interface, "frequency"])
        frequences =  re.findall(r"\d* : (\d*\.\d*) GHz", output.decode("utf-8"))
        for frequence in reversed(frequences):
            if float(frequence) >= 5.18:
                return True
        return False

    except subprocess.CalledProcessError:
        print(f"Error: Failed to retrieve wireless capabilities for {interface}.")
        return None

def aau_get_wireless_interfaces():
    try:
        output = subprocess.check_output('iwconfig | cut -d " " -f 1 | grep . || true', shell=True, stderr=subprocess.DEVNULL)
        interfaces = re.findall(r"(\w+)", output.decode("utf-8"))
        return interfaces
    except subprocess.CalledProcessError:
        print("Error: Failed to retrieve wireless interfaces.")
        return None

def aau_install_2_4GHz():
    for interface in aau_get_wireless_interfaces():
        if aau_5Ghz_capable (interface):
            return False
    return True

install_2_4Ghz = False
if aau_install_2_4GHz():
    Config.ssids.append('eduroam-2.4Ghz')
    install_2_4Ghz = True


Config.username = "396102@net.aau.dk"
Config.password = "sahsnxjydrhmjgi"
Config.silent = True

run_installer()

print ("Installation complete")
print ("\nThe installations is now finished.")
if install_2_4Ghz:
    print("Your PC cannot use 5 Ghz Wi-Fi, and you will therefore have lower perfomance.")

print("If you have any problems you can contact support on phone +45 9940 2020.")
